...
params <- list( share = .86, gamma = 8, P1.bar=1, P2.bar=1, betta=.99,
                rho=c(.9,.9, .9, .9), sig.eps=c(.01,.01, .005, .005), eta=2, 
                theta=.05 )
params.0 <- params
params.0$theta <- 0
...
sol.1.0.sym <- list(
  coeff=m.sym.ave.pair( sol.1.0$coeff, l.sym.ave, l.pairs ),
  coeff.cont=m.sym.ave.pair( sol.1.0$coeff.cont, l.sym.ave, l.pairs.cont ),
  opt=opt, params=params.0 )
...
# l.sym.ave <- list( ave=c(1,5,14,27),
...
baseline.sol.0 <- list( params=params.0, opt=sol.1$opt, 
...
sim.sol.1.0.sym <- sim.sol( sol.1.0.sym, n.sim, sim.exog )
...
colnames(pred) <- c( endog.names, cont.names )
pred[,fwd.vars] <- 
  euler_hat_grid( baseline.sol.0$coeff, baseline.sol.0$coeff.cont, 
                  sim.baseline.0, opt$lags, params, 
                  n.exog, n.endog, n.cont, opt$n.fwd, params$rho, 
                  params$sig.eps, 0, 1, upper, lower, opt$cheby, 
                  matrix(0,1,1,), TRUE, opt$n.quad, opt$model, opt$mono )
baseline.err.0 <- sim.baseline.0[,c( endog.names, cont.names )] - pred
...
euler.rmse <- rbind( 
  local=apply(baseline.err[,c(fwd.vars,'B22')],2, function(x) sqrt(sum(x^2))),
  global.1=apply(sol.1.err[,c(fwd.vars,'B22')],2, function(x) sqrt(sum(x^2))),
  global.2=apply(sol.2.err[,c(fwd.vars,'B22')],2, function(x) sqrt(sum(x^2))),
  global.1.0=apply(sol.1.0.err[,c(fwd.vars,'B22')],2, function(x) sqrt(sum(x^2))),
  global.2.0=apply(sol.2.0.err[,c(fwd.vars,'B22')],2, function(x) sqrt(sum(x^2))) )
